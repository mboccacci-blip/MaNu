...
export default function AchieveTab() {
  var { engine, goTab, tier, isDemo } = useEngine();
  ...
  // Canonical (pre-tax) return for display — always the fixed profile rate
  var retCanonicalRate = (adjProfiles.find(function(p){return p.name===retProfLabel}) || adjProfiles[2]).realReturn;
  ...
  return (
    <div className="fi">
      ...
      {magic.real>0&&ytr>0&&nEx>=0&&(mSav>0||nEx>0)?<>
        ...
        <Cd style={{padding:"20px"}}>
          <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
            ...
            <div style={{flex:"1 1 250px",textAlign:"center",padding:"20px 16px",borderRadius:14,background:baseProjected>=magic.real?"rgba(34,197,94,0.04)":"rgba(239,68,68,0.04)",border:"1px solid "+(baseProjected>=magic.real?"rgba(34,197,94,0.12)":"rgba(239,68,68,0.12)")}}>
              ...
              <div style={{fontSize:10,color:"#94a3b8",marginTop:8,lineHeight:1.5,fontStyle:"italic"}}>{lang==="en"?"This projection assumes investing at "+pct(retCanonicalRate)+" real ("+retProfLabel+") from now until retirement.":"Este ahorro proyectado asume que desde hoy hasta que te jubilás invertís al "+pct(retCanonicalRate)+" real ("+retProfLabel+")."}</div>
            </div>
        ...
        <div style={{padding:"12px 16px",borderRadius:10,background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.15)",fontSize:11,color:"#92400e",lineHeight:1.6,display:"flex",alignItems:"flex-start",gap:8,margin:"-4px 0 16px 0"}}><Icon name="info" size={14} weight="regular" style={{marginTop:1,flexShrink:0,color:"#d97706"}} /><span>{t('achieve.returnsDisclaimer')}</span></div>
        <Cd style={{padding:"20px"}}>
          <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
