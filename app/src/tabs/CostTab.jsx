  ...
  return (
    <div className="fi">
      <Cd><ST tip={t('cost.tip')} sub={t('cost.sub')}>{t('cost.title')}</ST>
        ...
        <div style={{fontSize:13,fontWeight:500,color:"#64748b",marginBottom:8}}>{t('cost.investProfile')}</div>
      ...
      {costInRet&&ytr>0&&<>
        <Cd glow="orange" style={{textAlign:"center",padding:"32px 24px"}}>
          ...
          <div style={{marginTop:12,padding:"8px 14px",borderRadius:10,background:"rgba(96,165,250,0.04)",border:"1px solid rgba(96,165,250,0.08)",fontSize:11,color:"#3b82f6",textAlign:"center"}}>{t('cost.allInTodayDollars',{profile:costInRet.prof.icon+" "+costInRet.prof.name,rate:pct(costInRet.prof.realReturn),years:ytr})}</div>
        </Cd>
