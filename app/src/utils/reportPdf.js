...
var STR = {
  es: {
    ...
    method3: 'Cada perfil utiliza un retorno nominal histórico de largo plazo. El retorno real se obtiene ajustando ese retorno por la inflación esperada (ecuación de Fisher). Los rendimientos pasados no garantizan resultados futuros. Si configurás un impuesto anual sobre los activos, también lo descontamos (drag fiscal).',
  ...
  en: {
    ...
    method3: 'Each profile uses a long-term historical nominal return. The real return is derived by adjusting for expected inflation (Fisher equation). Past performance does not guarantee future results. If you set an annual asset tax, we also subtract it (tax drag) in calculations.',
